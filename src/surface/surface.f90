...
!subroutine init_surface_calculator(self, num, vdwRad, dielectricConst, freeEnergyShift, &
...
subroutine update(self, num, xyz)
   ...
   !self%bornMat(:, :) = 0.0_wp
   !call self%addBornMatrix(num, xyz, self%bornMat)

end subroutine update


!subroutine addBornMatrix(self, num, xyz, bornMat)
!
...
!!> Calculate solvation energy
!subroutine getEnergyParts(self, qat, qsh, gborn, ghb, gsasa, gshift)
!
!   !> Error source
!   character(len=*), parameter :: source = 'solv_gbsa_getEnergy'
!
...
!   end if
   ...
   write(unit, '(8x, a, t40, f14.4, 1x, a)') &
      "Probe radius", self%probeRad_au, "/ Bohr"

   write(unit, '(8x, a )') repeat('-',54)
