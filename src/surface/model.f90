...
!> Initialize solvation model from input
subroutine initSolvModel(self, input, level)
   ...
   if (input%alpb) then
      call getParamFile(solvent, 'alpb', level, self%paramFile)
   end if

   !if (input%cosmo) then
   !   call getParamFile(solvent, 'cosmo', level, self%paramFile)
   !end if

   if (.not.allocated(self%paramFile) .and. (.not.input%cosmo)) then
      call getParamFile(solvent, 'gbsa', level, self%paramFile)
   end if

   if (allocated(self%paramFile)) then
      call readParamFile(self)
   else
      call loadInternalParam(self, solvent, level)
   end if
...
subroutine loadInternalParam(self, solvent, level)
   ...
   case (gbKernel%still)
      ...
      if (.not.allocated(param)) then
          write(stderr,*) "solvent: '"//solvent//"' is not parametrized"
          stop source
         return
      end if
   ...
   case (gbKernel%p16)
      ...
      elseif (level == 0) then
         ...
         select case(solvent)
         case('acetone');      param = xhcfflib_alpb_acetone
         case('acetonitrile'); param = xhcfflib_alpb_acetonitrile
         case('aniline');      param = xhcfflib_alpb_aniline
         case('benzaldehyde');      param = xhcfflib_alpb_benzaldehyde
         case('benzene');      param = xhcfflib_alpb_benzene
         case('dioxane');      param = xhcfflib_alpb_dioxane
         case('ethylacetate');      param = xhcfflib_alpb_ethylacetate
         case('furane');      param = xhcfflib_alpb_furane
         case('hexadecane');      param = xhcfflib_alpb_hexadecane
         case('nitromethane');      param = xhcfflib_alpb_nitromethane
         case('octanol');      param = xhcfflib_alpb_octanol
         case('woctanol');      param = xhcfflib_alpb_woctanol
         case('phenol');      param = xhcfflib_alpb_phenol 
         case('ch2cl2','dichlormethane'); param = xhcfflib_alpb_ch2cl2
         case('chcl3','chloroform');      param = xhcfflib_alpb_chcl3
         case('cs2');          param = xhcfflib_alpb_cs2
         case('dmso');         param = xhcfflib_alpb_dmso
         case('ether');        param = xhcfflib_alpb_ether
         case('h2o','water');  param = xhcfflib_alpb_water
         case('methanol');     param = xhcfflib_alpb_methanol
         case('ethanol');      param = xhcfflib_alpb_ethanol
         case('thf');          param = xhcfflib_alpb_thf
         case('toluene');      param = xhcfflib_alpb_toluene
         case('dmf');          param = xhcfflib_alpb_dmf
         case('nhexan','n-hexan','nhexane','n-hexane','hexane');
            param = xhcfflib_alpb_hexane
      end select
      else
...
!            if (iostat .ne. 0) then
!               write(stderr,*) solvent// " is neither a solvent name nor a dielectric constant  "
!               stop source
!               return
!            end if
!            if (param%epsv .eq. 0) then
!               write(stderr,*) "You really chose 0 as your dielectric constant?"
!               stop source
!               return
!            end if
!         end if
...
subroutine readParamFile(self)
   ...
   open(newunit=unit, file=self%paramFile, action='read')
   if (unit == -1) then
       write(stderr,*) "Could not open parameter file '"//self%paramFile//"'"
       stop source
      return
   end if
   ...
   if (statc /= 0) then
      write(stderr,*) "Could not read parameters from '"//self%paramFile//"'"
      stop source
      return
   end if
...
subroutine getParamFile(solvent, model, level, pfile)
...
subroutine newSolvationModel(self, model, num)
...
!   if (self%cosmo) then
!      allocate(cosmo)
!      call init_(cosmo, num, self%dielectricConst, self%nAng, self%bornScale, &
!         & self%vdwRad, self%surfaceTension, self%probeRad, srcut)
!      call move_alloc(cosmo, model)
!   else
      allocate(born)
      call init_(born, num, self%vdwRad, self%dielectricConst, &
...
subroutine newBornModel(self,  model, num)
   ...
   call init_(model, num, self%vdwRad, self%dielectricConst, &
      & self%freeEnergyShift, self%descreening, self%bornScale, &
      & self%bornOffset, self%surfaceTension, self%probeRad, lrcut, srcut, &
      & self%nAng, self%hBondStrength, self%temperature, self%kernel, &
      & self%alpb, self%ionStrength, self%ionRad)

   model%solvent     = self%solvent
   model%paramFile   = self%paramFile
   model%state       = self%state
   model%temperature = self%temperature
   model%density     = self%density
   model%molarMass   = self%molarMass
   model%bornOffset  = self%bornOffset
   model%ionStrength = self%ionStrength
   model%nAng        = self%nAng
