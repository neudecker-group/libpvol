...
class LibpvolCalculator:
    ...
    def __init__(self, nat: int, at: np.ndarray, xyz: np.ndarray, pressure: float,
        ...
        self._energy = None
        self._forces = None
        self._calculator = initialize_calculator(nat, at, xyz, pressure,
    ...
    def energy(self):
        ...
        if self._energy == None:
            raise RuntimeError('Run singlepoint before accessing energy!')
        return self._energy
    ...
    def forces(self):
        ...
        if self._forces == None:
            raise RuntimeError('Run singlepoint before acessing gradient!')
        return self._forces 
    ...
    def calculate_single_point(self, nat: int, at: np.ndarray, xyz: np.ndarray):
        ...
        Perform a single-point energy and force calculation. \n
            ...
            :returns: iostat -- I/O status indicating success (0) or failure (non-zero).
            :rtype: int
        """
        self._energy = np.zeros(1, dtype=np.float64)  # Assuming energy is a scalar
        self._forces = np.zeros((nat, 3), dtype=np.float64)  # Force array of size (nat, 3)
        ...
        self._calculator.singlepoint(nat, at, xyz, self._energy, self._forces, iostat)
